...
`curated` ejecuta evaluación interna y transferencia cruzada; `combined` prueba una mezcla de campañas conocidas; `temporal` evalúa cuentas nuevas en la ventana posterior. `coordinacion.py` construye redes exploratorias sin usar las etiquetas para crear enlaces.

## Interpretación

Las etiquetas distinguen cuentas incluidas en una operación documentada y cuentas de referencia. No equivalen por sí solas a bot/humano, identidad verdadera/falsa ni contenido verdadero/desinformación. Los clasificadores y los grafos se evalúan por separado; estos comandos no constituyen todavía una validación de fusión real.
